        ...
        <tr v-for="(item, i) in demoQues" :key="i" >
            <td class="text-left"> {{i+1}} </td>
              <td class="text-left" style="font-size:18px">
                ...
                <span v-for="(phrase, index) in item.queS.split(' ')" :key="index">  
                  <span v-if="phrase!='___'"> {{phrase}} </span>
                  <input :placeholder="item.uans" type="text"
            ...
            <td class="text-right"> 
              ...
              <v-icon color="orange" @click="playSegment(item)"> mdi-play-circle </v-icon> </td>
        </tr>
        </tbody>
    </template>
    </v-simple-table>    
...
<v-row>
    ...
    <v-col cols="4">
        <v-select :return-object="true" class="mx-2" :items="duotingDemo" v-model="selUnit" item-text="queName" @change="unitChange()" label="Exercises"></v-select>
    </v-col>
    <v-col cols="4">
      <v-text-field outlined v-model="practiceURL"></v-text-field>
    </v-col>
</v-row>
</v-container>
...
export default {
  ...
  methods: {
    unitChange(){
      ...
      this.practiceURL = 'https://firechrome-817a7.web.app/practice/?id=' + this.selUnit.id
    },
    classChange(){
      this.duotingDemo = []
    ...
    getAllDemoUnits(){
      this.$fireStore.collection('-video_practice').where('inst', '==', this.$store.state.user.email).get().then(query =>{
        //console.log(query)
        if (query.empty) {
          this.sbcolor = 'warning'
